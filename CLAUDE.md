...
### Package Structure
- **Sources/ASC/**: Main library (18 files, ~2,915 lines)
  - **Auth/**: Authentication (4 files, 236 lines)
    - `AuthInterceptor.swift` - Token refresh (98 lines)
    - `AuthToken.swift` - Auth tokens (56 lines)
    - `OAuthAuthenticator.swift` - OAuth authentication (39 lines)
    - `TokenStorage.swift` - Token storage protocol (43 lines)
  - **Client/**: NetworkClient and components (3 files, ~1,180 lines)
  ...
  - **Extensions/**: Extensions for external types (2 files, ~75 lines)
    ...
    - `RetryPolicy+Extension.swift` - Retry policy presets extension (60 lines)
  - **Utils/**: Utility classes (3 files, ~593 lines)
